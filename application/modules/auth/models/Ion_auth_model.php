...
class Ion_auth_model extends CI_Model
{
	...
	public function hash_password_db($id, $password, $use_sha1_override = FALSE)
	{
		...
		$query = $this->db->select('password')
			...
			->limit(1)
			->order_by('id', 'desc')
			->get($this->tables['user_credentials']);
	...
	public function reset_password($identity, $new)
	{
		...
		$this->db->update($this->tables['user_credentials'], [
			'password' => $new
	...
	public function change_password($identity, $old, $new)
	{
		...
		$query = $this->db->select($this->tables['users'] . ".id, " . $this->tables['user_credentials'] . ".password, salt")
			->join($this->tables['user_credentials'], $this->tables['user_credentials'] . ".user_id=" . $this->tables['users'] . ".id", 'inner',)
			->where($this->identity_column, $identity)
			->limit(1)
			->order_by($this->tables['users'] . '.id', 'desc')
			->get($this->tables['users']);
		...
		if ($old_password_matches === TRUE) {
			...
			$successfully_changed_password_in_db = $this->db->update($this->tables['user_credentials'], $data, array('user_id' => $user->id));
	 ...
	 * @author Mathew
	 **/
	public function register($identity, $password, $email, $additional_data = array(), $intent = 'user', $autoApprove = FALSE)
	{
		...
		if ($this->identity_check($identity)) {
			...
			return FALSE;
		} elseif (!$this->config->item('default_group', 'ion_auth')) {
		...
		$data = array(
			...
			'active'     => 1,
			'profile_image' => $additional_data['profile_image']
		);
		...
		//Stored user_accounts based user_id
		$this->user_model->createUserDefaults($id);
		$credential = [
			'user_id' => $id,
			'password' => $password
		];
		$this->db->insert($this->tables['user_credentials'], $credential);
		...
		if (!empty($groups)) {
			...
			if ($intent != 'user') {
				$query = $this->db->get_where($this->tables['groups'], array('name' => $intent), 1)->row();
				if ($query->id) {
					$status = ($autoApprove === TRUE) ? 1 : 3;
					$this->add_to_group($query->id, $id, $status);
				}
			}
		}
	...
	public function login($identity, $password, $remember = FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', email, ' . $this->tables['users'] . '.id,' . $this->tables['user_credentials'] . '.password, active, last_login')
			->where($this->identity_column, $identity)
			->limit(1)
			->order_by($this->tables['users'] . '.id', 'desc')
			->join($this->tables['user_credentials'], $this->tables['user_credentials'] . ".user_id=" . $this->tables['users'] . ".id", 'inner')
	 ...
	 * @author Ben Edmunds
	 **/
	public function add_to_group($group_ids, $user_id = false, $status = 1)
	{
		...
		foreach ($group_ids as $group_id) {
			if ($this->db->insert($this->tables['users_groups'], array($this->join['groups'] => (float)$group_id, $this->join['users'] => (float)$user_id, 'status' => $status))) {
