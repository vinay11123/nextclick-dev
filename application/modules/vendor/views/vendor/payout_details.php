			...
			<div class="col-md-12">
				<div class="main-card card">
					...
					<div class="card-body">
						...
						<form class="form-inline" novalidate="" action="<?php echo base_url();?>vendor/payout_detials/details/0?vendor_user_id=<?=$v_did;?>" method="post">
                          <div class="form-group col-3">
                            <label for="start_date">Start date:</label>
                            <input type="text" name="start_date" value="<?php echo (empty($start_date))? '' : $start_date;?>" class="form-control" id="start_date">
                          </div>
                          <div class="form-group col-3">
                            <label for="end_date">End date:</label>
                            <input type="text" name="end_date" value="<?php echo (empty($end_date))? '' : $end_date;?>" class="form-control" id="end_date">
                          </div>
                          <div class="form-group col-3">
                            <label for="noofrows">Rows count:</label>
                            <input type="text" name="noofrows" value="<?php echo (empty($noofrows))? '' : $noofrows;?>" class="form-control" id="noofrows">
                          </div>
                          <div class="col-md-12 mt-3">
                          		<button type="submit" class="btn btn-lg btn-outline-primary">Submit</button>
                          		<a href="<?php echo base_url();?>vendor/payout_detials/details/0?vendor_user_id=<?=$v_did;?>" class="btn btn-lg btn-outline-primary">Clear</a>
...
<?php if($txn['track_id']!=null){ 
											$this->db->select('*');
											$this->db->from('ecom_orders');
											$this->db->join('users', 'ecom_orders.created_user_id=users.id');
											$this->db->where('track_id', $txn['track_id']);
											$query = $this->db->get();
											 $rows = $query->result();
											foreach ($rows as $row){
													
												?>
												<div class="widget-heading"><?php echo (empty($row->display_name))? $row->first_name : $row->display_name?></div>
												<div class="widget-subheading opacity-7"><?php echo $row->phone?></div>
											<?php } } else{ 
											 ?>
								...
								<td class="text-center">
										...
										<a href="<?php echo base_url();?>vendor/payout_detials/edit?id=<?=$enorder_id;?>" class="mr-2" > <i class="fas fa-eye"></i></a>
										<?php } ?>
								</td>
								</tr>
								<?php endforeach;else :?>
