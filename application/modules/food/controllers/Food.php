...
ORDER BY eo.id DESC LIMIT " . $rowno . ',' . $rowperpage)->result_array();
        ...
        if ($type == 'edit') {
            ...
            $this->data['orderst'] = $this->db->query("SELECT eo.*,eo.total as grand_total,ua.address,u.first_name,u.last_name,u.email,u.phone,vl.name as vandor_name,vl.email as vendor_email, vl.secondary_contact as vendor_phone ,dm.name as delivery_mode_name,ep.payment_method_id,pm.name as payment_method_name FROM ecom_orders as eo 
        join users_address as ua on ua.id =eo.shipping_address_id 
        join users as u on u.id = eo.created_user_id 
        join vendors_list as vl on vl.vendor_user_id = eo.vendor_user_id 
        join ecom_payments as ep on ep.id = eo.payment_id
