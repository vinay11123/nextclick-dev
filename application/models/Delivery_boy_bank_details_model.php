...
class Delivery_boy_bank_details_model extends MY_Model
{
    public $rules;
    public $user_id =1; 
    ...
    {
        parent::__construct();
        $this->table = 'delivery_partner_bank_details';
        $this->primary_key = 'id';
    ...
    public function checkUpdateExternalAccount($userID, $externalID){
        try{
            $externalFundAccount = $this->createFundAccount($userID, $externalID);
            $this->update([
    ...
    public function createFundAccount($userID, $externalID){
        try{
            $bankDetails = $this->fields('bank_name, ifsc, ac_number')->where([
                'user_id' => $userID,
                'status'=> 1
            ])->get();
            ...
            switch($accountType){
                ...
                case 'bank_account': 
                    ...
                    $accountDetails['bank_account'] = array(
                        ...
                        'account_number'=> $bankDetails['ac_number']
                    );
                    break;
                default:
                    break;
