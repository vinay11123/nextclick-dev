...
$config['tables']['user_credentials']           = 'user_credentials';
/*
...
$config['salt_prefix']    = version_compare(PHP_VERSION, '5.3.7', '<') ? '$2a$' : '$2y$';
    ...
    /*groups and ids*/
    $config['groups'] = $CI->db->query('SELECT * FROM `groups` WHERE 1')->result_array();
    foreach($config['groups'] as $group){
        $config[$group['name'].'_group_id'] = $group['id'];
    }
    
    /*Services and ids*/
    $config['services'] = $CI->db->query('SELECT * FROM `services` WHERE 1')->result_array();
    foreach($config['services'] as $service){
        $config[$service['key'].'_service_id'] = $service['id'];
    }
    
    
    /*Emial settings*/
    $results = $CI->db->query("SELECT `key`, `value` FROM `settings` WHERE `key` IN('smtp_port', 'smtp_host', 'smtp_username', 'smtp_password')")->result();
    $email_settings = array();
    foreach($results as $r) {
        $email_settings[strtolower($r->key)] =  $r->value;
    }
    // echo "<pre>"; print_r($site_settings); die();
    $CI->config->set_item('email_settings', (object)$email_settings);
    
    /*SMS settings*/
    $results = $CI->db->query("SELECT `key`, `value` FROM `settings` WHERE `key` IN('sms_username', 'sms_sender', 'sms_hash')")->result();
    $sms_settings = array();
    foreach($results as $r) {
        $sms_settings[strtolower($r->key)] =  $r->value;
    }
    // echo "<pre>"; print_r($site_settings); die();
    $CI->config->set_item('sms_settings', (object)$sms_settings);
    
    /*Site settings*/
    $results = $CI->db->query("SELECT `key`, `value` FROM `settings` WHERE `key` IN('system_name', 'system_title', 'address', 'mobile', 'privacy', 'terms', 'facebook', 'twiter', 'youtube', 'skype', 'pinterest')")->result();
    $site_settings = array();
    foreach($results as $r) {
        $site_settings[strtolower($r->key)] =  $r->value;
    }
    // echo "<pre>"; print_r($site_settings); die();
 ...
 */
$config['super_admin_user_id']        = 1;       // Site Title, example.com
$config['site_title']                 = $site_settings->system_name;       // Site Title, example.com
$config['admin_email']                = $email_settings->smtp_username; // Admin Email, admin@example.com
$config['default_group']              = 'user';           // Default group, use name
$config['admin_group']                = 'admin';             // Default administrators group, use name
$config['identity']                   = 'phone';             // You can use any unique column in your table as identity column. The values in this column, alongside password, will be used for login purposes
$config['min_password_length']        = 6;                   // Minimum Required Length of Password
$config['max_password_length']        = 20;                  // Maximum Allowed Length of Password
$config['email_activation']           = FALSE;               // Email Activation for registration
$config['conn']   = mysqli_connect($CI->db->hostname, $CI->db->username, $CI->db->password, $CI->db->database);
